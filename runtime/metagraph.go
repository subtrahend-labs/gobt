...
// GetAllMetagraphs retrieves all metagraphs
func GetAllMetagraphs(c *client.Client, blockHash *types.Hash) ([]*Metagraph, error) {
	var encodedResponse []byte
	err := c.Api.Client.Call(
		&encodedResponse,
	...
	if len(encodedResponse) == 0 {
		fmt.Println("No metagraphs found")
		return []*Metagraph{}, nil
	}

	// Debug information
	fmt.Printf("Received %d bytes for all metagraphs\n", len(encodedResponse))

	// Try as a raw byte vector first
	var rawBytes []byte
	err = codec.Decode(encodedResponse, &rawBytes)
	if err == nil {
		fmt.Println("Successfully decoded as raw bytes, attempting to decode content...")
		var metagraphs []*Metagraph
		err = codec.Decode(rawBytes, &metagraphs)
		if err != nil {
			return nil, fmt.Errorf("failed to decode metagraphs from raw bytes: %v", err)
		}
		return metagraphs, nil
	}

	// Direct decoding attempt
	var metagraphs []*Metagraph
	err = codec.Decode(encodedResponse, &metagraphs)
	if err != nil {
		return nil, fmt.Errorf("failed to decode metagraphs directly: %v", err)
	}
